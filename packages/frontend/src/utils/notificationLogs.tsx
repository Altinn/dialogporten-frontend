...
export const labelFor = (namespace: 'type' | 'channel', value: string): string => {
...
const latest = (a: string, b: string): string => (new Date(b).getTime() > new Date(a).getTime() ? b : a);
...
export const groupNotificationLogs = (notificationLogs: NotificationLog[]): NotificationLogGroup[] => {
  ...
  for (const log of notificationLogs) {
    const sentAt = log.requestedSendTime ?? log.lastUpdateTime;
    if (!sentAt) continue;

    const deliveredAt = log.lastUpdateTime ?? sentAt;
    const type = normalize(log.type);
    const channel = normalize(log.channel);
    const key = [type, channel, log.transmissionId ?? '', sentAt].join('|');
    const existing = groups.get(key);

    if (existing) {
      existing.date = latest(existing.date, deliveredAt);
...
export const getNotificationLogEntries = ({
  ...
  return groupNotificationLogs(notificationLogs).map((group) => {
    const summary = (
      <>
