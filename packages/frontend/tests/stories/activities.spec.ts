...
test.describe('Activity history - transmissions and activities', () => {
  test('basic navigation', async ({ page }) => {
    ...
    await page.getByText('Meldingen ble sendt.').click();
    await expect(page.getByText('Meldingen ble sendt.')).toBeVisible();
    await page.getByText('Meldingen ble åpnet.').click();
    await page.getByRole('button', { name: 'Tittel', exact: true }).click();
    await page.getByText('Oppsummering').click();
    await page.getByText('Denne meldingen er utløpt.').click();
