...
test.describe('Date filter, system date set 2024', () => {
  ...
  test('Date filter - quick filters functionality', async ({ page }) => {
    ...
    await expect(page.getByRole('menuitemcheckbox', { name: 'I dag' })).toBeVisible();
    await expect(page.locator('li').filter({ hasText: 'I dag' }).locator('span > span').nth(4)).toHaveText('3');

    await expect(page.getByRole('menuitemcheckbox', { name: 'Denne måneden' }).locator('div')).toBeVisible();
    await expect(page.locator('li').filter({ hasText: 'Denne måneden' }).locator('span > span').nth(4)).toHaveText('3');

    await expect(page.getByRole('menuitemcheckbox', { name: 'Siste tolv måneder' }).locator('div')).toBeVisible();
    await expect(page.locator('li').filter({ hasText: 'Siste tolv måneder' }).locator('span > span').nth(4)).toHaveText(
      '9',
    );

    await page.getByRole('menuitemcheckbox', { name: 'I dag' }).locator('div').click;
